...
export function createOpenAIPlan(input: PlanInput) {
  return requestStructuredRecipes(
    [
      ...
      input.mode === "ingredients" ? `规划方式：${input.planScope === "meal" ? "可拆成一餐多道" : "组合成一道菜"}` : "",
      input.mode === "ingredients" ? `家中已有基础调料：${input.pantry || "无"}` : "",
      input.mode === "ingredients" && input.zeroPurchase
        ? "零采购是硬约束：不得加入已有食材和基础调料之外的任何原料；必须使用全部已有食材。"
        : "",
      `人数：${input.servings}`,
      `口味偏好：${input.taste || "家常"}`,
      `过敏原或绝对禁忌：${input.allergens || "无"}`,
      `总时间上限：${input.maxMinutes} 分钟`,
      "指定菜名模式不得为了迎合时间上限而改变菜品身份；若真实时间更长，应给出真实 totalMinutes。食材模式不得自行假设基础调料。totalMinutes 必须包括准备和烹饪时间。",
    ].join("\n"),
    input.servings,
    input.maxMinutes,
  );
...
export function createOpenAIReplan(input: ReplanInput) {
  ...
  return requestStructuredRecipes(
    [
      ...
      `总时间上限：${input.maxMinutes} 分钟。`,
      input.zeroPurchase ? `零采购约束：只能使用库存 ${input.inventory || input.recipe.inventoryCoverage?.used.join("、") || "原菜谱库存"} 与基础调料 ${input.pantry || input.recipe.inventoryCoverage?.pantryUsed.join("、") || "无"}。` : "",
    ].join("\n"),
